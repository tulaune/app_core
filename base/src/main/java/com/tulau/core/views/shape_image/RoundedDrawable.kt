...
package com.tulau.core.views.shape_image
...
class RoundedDrawable(val sourceBitmap: Bitmap) : Drawable() {
    ...
    override fun draw(canvas: Canvas) {
        if (mRebuildShader) {
            val bitmapShader = BitmapShader(sourceBitmap, mTileModeX, mTileModeY)
    ...
    fun setScaleType(scaleType: ScaleType?): RoundedDrawable {
        var itScaleType = scaleType
        if (itScaleType == null) {
            itScaleType = ScaleType.FIT_CENTER
        }
        if (mScaleType != itScaleType) {
            mScaleType = itScaleType
            updateShaderMatrix()
        }
        return this
    }
    ...
    fun setTileModeY(tileModeY: Shader.TileMode?): RoundedDrawable {
        if (tileModeY != null && mTileModeY != tileModeY) {
